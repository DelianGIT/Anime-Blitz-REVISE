...
function Hitbox.GetHits(self: Hitbox): Hits
	...
	if visualization then
		visualization.CFrame = geometry.CFrame
		visualization.Size = geometry.Size
...
function Module.new(geometry: Geometry, hitboxParams: HitboxParams, requestParams: RequestParams): Hitbox
	if typeof(geometry) ~= "Instance" or not geometry:IsA("BasePart") then
		error(`Invalid geometry for PreciseSpatialQuery hitbox: {geometry}`)
	end

	local visualization: Part?
	if Visualization.IsEnabled(hitboxParams.Visualize) then
		visualization = Visualization.PreciseSpatialQuery(geometry, false)
	end
