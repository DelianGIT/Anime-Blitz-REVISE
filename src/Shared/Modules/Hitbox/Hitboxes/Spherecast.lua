...
type CustomRaycastResult = BaseFunctions.CustomRaycastResult
...
function Hitbox.GetHit(self: Hitbox): CustomRaycastResult?
	...
	local radius: number = geometry.Radius
	...
	if visualization then
		...
		visualization.Size = Vector3.new((distance or direction.Magnitude) :: number, radius, radius)
...
function Module.new(geometry: Geometry, hitboxParams: HitboxParams, requestParams: RequestParams): Hitbox
	...
	if not origin or not direction or radius then
		error(`Invalid geometry for Spherecast hitbox: {geometry}`)
	end

	local visualization: Part?
	if Visualization.IsEnabled(hitboxParams.Visualize) then
		visualization = Visualization.Spherecast(origin :: Vector3, direction, radius, false)
	end
